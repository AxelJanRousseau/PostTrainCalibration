def create_unet_like_model(
    ...
    :param dropout_during_inference:
    :param output_size:
    ...
    import numpy as np
    # from ..components.calculate_gpu_memory import get_model_memory_usage
    from tensorflow.keras.models import Model
    from tensorflow.keras.layers import Input, Dropout, MaxPooling3D, Concatenate, Multiply, Add, Reshape, Lambda, \
        AveragePooling3D
    ...
    def normalization_function():
        if batch_normalization_on_input or batch_normalization:
            ...
            normalization_function_ = Lambda(lambda x: (x - K.mean(x, axis=[1, 2, 3], keepdims=True)) / (
                        K.std(x, axis=[1, 2, 3], keepdims=True) + K.epsilon()))
            # normalization_function_ = InstanceNormalization(axis=-1)
    ...
    def introduce_metadata(path, i):
        ...
        for j, (m_n_f, m_d) in enumerate(zip(metadata_number_features[i], metadata_dropout[i])):
            ...
            metadata_path = Conv3D(filters=m_n_f, kernel_size=(1, 1, 1), padding=padding,
                                   kernel_initializer=kernel_initializer,
                                   kernel_regularizer=regularizers.l1_l2(l1_reg, l2_reg))(metadata_path)
    ...
    if dropout_center and dropout_decoders:
        raise ValueError("choose only one dropout setting")
    ...
    for p in range(nb_pathways - 1):
        if not [f_next % f for f, f_next in
                zip(subsample_factors_per_pathway[p], subsample_factors_per_pathway[p + 1])] == [0, 0, 0]:
            raise ValueError(
    ...
    if metadata_sizes in [None, []]:
        ...
        if not len(metadata_sizes) == len(metadata_dropout) == len(metadata_number_features) == len(
                metadata_at_common_pathway_layer):
    ...
    if (batch_normalization_on_input or batch_normalization) and (
            instance_normalization_on_input or instance_normalization):
    ...
    if verbose:
        ...
        print("input size:\t{}\t(inferred with theoretical field of view (less meaningful if padding='same'))".format(
            input_size))
    ...
    for i, (s_f_p_p, k_s_p_p) in enumerate(
            zip([(1, 1, 1)] + subsample_factors_per_pathway, [[[], []]] + kernel_sizes_per_pathway)):
    ...
    for i, (s_f_p_p, k_s_p_p) in reversed(list(enumerate(
            zip([(1, 1, 1), (1, 1, 1)] + subsample_factors_per_pathway[:-1], [[[], []]] + kernel_sizes_per_pathway)))):
        ...
        if i > 0:
            ...
            for j, (k_s, n_f) in enumerate(zip(k_s_p_p[0], n_f_p_p[0])):
                if j == 0 and (residual_connections or dense_connections):
                    ...
                    shortcut = Cropping3D(
                        [int((l - r) / 2) for l, r in zip(K.int_shape(shortcut)[1:-1], K.int_shape(path)[1:-1])])(
                        shortcut)
                    shortcut = Concatenate(axis=-1)([path, shortcut])
                    path = shortcut
                if ((i > 1 or j > 0) and (batch_normalization or instance_normalization)) and (
                        not relaxed_normalization_scheme or j == len(k_s_p_p[0]) - 1):
                    path = normalization_function()(path)
                if i > 1 or j > 0:
                    path = activation_function()(path)
                path = Conv3D(filters=n_f, kernel_size=k_s, padding=padding, kernel_initializer=kernel_initializer,
                              kernel_regularizer=regularizers.l1_l2(l1_reg, l2_reg))(path)
                if j == len(k_s_p_p[0]) - 1 and residual_connections:
                    shortcut = Cropping3D(
                        [int((l - r) / 2) for l, r in zip(K.int_shape(shortcut)[1:-1], K.int_shape(path)[1:-1])])(
                        shortcut)
                    if K.int_shape(path)[-1] != K.int_shape(shortcut)[-1]:
                        shortcut = Conv3D(filters=K.int_shape(path)[-1], kernel_size=(1, 1, 1), padding=padding,
                                          kernel_initializer=kernel_initializer,
                                          kernel_regularizer=regularizers.l1_l2(l1_reg, l2_reg))(shortcut)
                    path = Add()([path, shortcut])


        # Center dropout
        if i > (nb_pathways-1)-2 and dropout_center:
            path = Dropout(dropout_common_pathway[0])(path,training=True) if dropout_during_inference else Dropout(dropout_common_pathway[0])(path)
    ...
    for i, m_a_c_p_l in enumerate(metadata_at_common_pathway_layer):
        ...
        # Upwards
    prev_s_f_p_p = np.array(subsample_factors_per_pathway[-1])
    for i, (s_f_p_p, k_s_p_p, n_f_p_p) in reversed(list(enumerate(
            zip([(1, 1, 1), (1, 1, 1)] + subsample_factors_per_pathway[:-1], [[[], []]] + kernel_sizes_per_pathway,
                [[[], []]] + number_features_per_pathway)))):
        path = AveragePooling3D(np.abs(path_left_output_positions[i] - path_right_input_positions[i]) + 1,
                                strides=(1, 1, 1))(path)
        if i > 0:
            if i < nb_pathways:
                path_left = Cropping3D([int((l - r) / 2) for l, r in
                                        zip(K.int_shape(path_left_output_paths[i])[1:-1], K.int_shape(path)[1:-1])])(
                    path_left_output_paths[i])

                path = Concatenate(axis=-1)([path_left, path])
                # Center dropout or decoder dropout
                if dropout_decoders or (i > (nb_pathways-1) - 2 and dropout_center):
                    path = Dropout(dropout_common_pathway[0])(path,
                                                              training=True) if dropout_during_inference else Dropout(
                        dropout_common_pathway[0])(path)
            for j, (k_s, n_f) in enumerate(zip(k_s_p_p[1], n_f_p_p[1])):
                ...
                if (batch_normalization or instance_normalization) and (
                        not relaxed_normalization_scheme or j == len(k_s_p_p[1]) - 1):
                    path = normalization_function()(path)
                path = activation_function()(path)
                path = Conv3D(filters=n_f, kernel_size=k_s, padding=padding, kernel_initializer=kernel_initializer,
                              kernel_regularizer=regularizers.l1_l2(l1_reg, l2_reg))(path)
    ...
    for i, (n_f_c_p, k_s_c_p, d_c_p) in enumerate(
            zip(number_features_common_pathway, kernel_sizes_common_pathway, dropout_common_pathway)):
        ...
        if (batch_normalization or instance_normalization) and not relaxed_normalization_scheme:
            path = normalization_function()(path)
        path = activation_function()(path)
        if d_c_p and not dropout_center:
            path = Dropout(d_c_p)(path, training=True) if dropout_during_inference else Dropout(d_c_p)(path)
        path = Conv3D(filters=n_f_c_p, kernel_size=k_s_c_p,
                      activation=activation_final_layer if i + 1 == len(number_features_common_pathway) else None,
    ...
    if mask_output:
        ...
        if activation_final_layer == 'linear':
            path = Add()([path, mask_path])
        else:
    ...
    if verbose:
        ...
        # print("With a batch size of 1 this model needs {} GB on the GPU.".format(get_model_memory_usage(1, model)))
